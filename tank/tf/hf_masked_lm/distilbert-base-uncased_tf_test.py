...
class DistilBertModuleTester:
    ...
    def create_and_check_module(self, dynamic, device):
        ...
        if shark_args.save_mlir == True or shark_args.save_vmfb == True or self.save_temps == True:
            repro_path = f"./shark_tmp/distilbert_base_uncased_tf_{dynamic}_{device}"
        ...
        if self.save_temps == True:
            ...
            with ireec.tools.TempFileSaver(temp_dir):
                mlir_importer = SharkImporter(model, (input,), frontend="tensorflow")
                mlir_module, func_name = mlir_importer.import_mlir(is_dynamic=dynamic, tracing_required=False)
                shark_module = SharkInference(mlir_module, func_name, device=device, mlir_dialect="mhlo")
